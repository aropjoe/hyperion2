...
def group_purchases_by_date(data_frame):
    if "PO CREATION DATE" in data_frame.columns and "PO NUMBER" in data_frame.columns:
        ...
        grouped_data = (
            data_frame.groupby("PO CREATION DATE")["PO NUMBER"].sum().reset_index()
        )
        data = {
...
def group_purchases_by_supplier(data_frame):
    if "SUPPLIER ACCOUNT" in data_frame.columns and "PO NUMBER" in data_frame.columns:
        data_frame = data_frame.dropna()
        grouped_data = (
            data_frame.groupby("SUPPLIER ACCOUNT")["PO NUMBER"].sum().reset_index()
        )
        data = {
...
def group_amount_by_supplier(data_frame):
    if (
        "SUPPLIER ACCOUNT" in data_frame.columns
        and "TOTAL LINE AMOUNT" in data_frame.columns
    ):
        data_frame = data_frame.dropna()
        grouped_data = (
            data_frame.groupby("SUPPLIER ACCOUNT")["TOTAL LINE AMOUNT"]
            .sum()
            .reset_index()
        )
        data = {
...
def group_purchases_by_line_number(data_frame):
    if "LineNumber" in data_frame.columns and "TOTAL LINE AMOUNT" in data_frame.columns:
        data_frame = data_frame.dropna()
        grouped_data = (
            data_frame.groupby("LineNumber")["TOTAL LINE AMOUNT"].sum().reset_index()
        )
        data = {
...
def group_purchases_by_part_number(data_frame):
    if (
        "Part Number" in data_frame.columns
        and "TOTAL LINE AMOUNT" in data_frame.columns
    ):
        data_frame = data_frame.dropna()
        grouped_data = (
            data_frame.groupby("Part Number")["TOTAL LINE AMOUNT"].sum().reset_index()
        )
        data = {
